    ...
    with st.form("input_form"):
        ...
        with a1:
            ...
            height = st.number_input(
                "Height (CM)", min_value=1.0, value=170.0,
        ...
        with a2:
            ...
            weight = st.number_input(
                "Weight (KG)", min_value=1.0, value=55.0,
                placeholder="Please input your weight in kilogram"
            )

        if use_country:
            country = st.selectbox(
                "Country", countries, index=None,
                placeholder="Select where you live, if not listed select 'Other'"
            )
            if country is None:
        ...
        with b1:
            blood_pressure = st.text_input(
                "Blood Pressure (Systolic/Diastolic)", placeholder="example: 140/80"
            )
            if blood_pressure and not is_valid_bp_format(blood_pressure):
                st.warning("Format must be like 120/80")

            triglycerides = st.number_input("Triglycerides (mg/dL)", min_value=0, value=150)
        ...
        diet = st.selectbox(
            "Diet Type",
            options=list(diet_labels.keys()),
            format_func=lambda x: diet_labels[x],
            index=None,
            placeholder="Select your diet type"
        )

        stress_level = st.slider("Stress Level (1 = Low, 10 = High)", min_value=1, max_value=10, value=5)
        physical_activity_days_per_week = st.slider("Physical Activity Per Week (Days)", min_value=0, max_value=7, value=3)
        sleep_hours_per_day = st.slider("Sleep Duration Per Day (Hours)", min_value=0.0, max_value=24.0, value=7.0, step=0.5)
        exercise_hours_per_week = st.slider("Exercise Per Week (Hours)", min_value=0.0, value=4.0, max_value=24.0 * 7.0, step=0.25)
        sedentary_hours_per_day = st.slider("Sedentary Per Day (Hours)", min_value=0.0, max_value=24.0, value=6.0, step=0.5)

        submitted = st.form_submit_button(
            "Submit", icon=":material/send:", icon_position="right", width="stretch"
        )
